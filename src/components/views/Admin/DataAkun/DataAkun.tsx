...
const DataAkun: React.FC = () => {
  ...
  const fetchUsers = async (page: number = 1, search: string = "") => {
    try {
      ...
      // Add minimal delay for loading state visibility
      ...
      if (response.data.length === 0 && page > 1) {
        ...
        const prevResponse = await getUsers({ page: prevPage, limit: 50, search });
        setUsers(prevResponse.data);
        setPagination({
  ...
  const handleModalSubmit = async (data: UserSubmitData) => {
    try {
      ...
      if (err.message.includes("tidak memiliki izin") || err.message.includes("Sesi Anda telah berakhir")) {
        setIsModalOpen(false); // Close modal only for auth-related errors
      }
    } finally {
  ...
  return (
    <PageContainer>
      ...
      {loading || pageLoading ? (
        ...
        <>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            {/* Empty State */}
            {users.length === 0 && !loading && pagination.total === 0 && (
      ...
      <CreateEditUserModal
        ...
        onClose={() => {
          setIsModalOpen(false);
          setError(null); // Clear any existing errors when closing modal
        }}
